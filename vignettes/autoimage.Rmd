...
across the plot area. We provide one solution for this issue that utilizes the `map.wrap.poly` function
in the **maps** package.

```{r, fig.height = 5, fig.width = 4}
worldpoly <- maps::map("world", plot = FALSE, wrap = TRUE)
worldpoly <- maps::map.wrap.poly(worldpoly, range(lon),
                                 poly = FALSE,
                                 antarctica = -89.5)
autoimage(x = lon, y = lat, z = tasmax[,,1], 
          lines = list(x = worldpoly$x, y = worldpoly$y),
