test_that("sanity checks for arg.check.ptext arguments", {
  ...
  expect_error(arg.check.ptext(list(x = 1, y = 1:2), NULL, 
                               labels = letters[1], proj = "none"))
  expect_error(arg.check.ptext(list(x = 1, y = "jim"), NULL, 
                               labels = letters[1], 
    proj = "none"), "x and y should be numeric")
  expect_error(arg.check.ptext(x = 1, y = "jim", labels = letters[1], 
                               proj = "none"), 
    "x and y should be numeric")
  expect_error(arg.check.ptext(x = 1, y = 1:2, labels = letters[1], 
                               proj = "none"), 
  ...
  expect_error(arg.check.ptext(x = 1:2, y = 1:2, labels = 1:2, 
                               proj = "none"), 
