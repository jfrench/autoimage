...
test_that("autoimage.xyz.setup detects correct type of problem", {
  ...
  expect_message(autoimage.xyz.setup(1:nr, 1:nc, tasmax[, , 1:2], 
                                     tx, ty, arglist, TRUE), 
...
test_that("sanity checks for autoimage.xyz.setup", {
  ...
  # regular grid okay
  expect_message(autoimage.xyz.setup(x = 1:nr, y = 1:nc, 
                                     z = tasmax[, , 1], "", "", 
                                     arglist, TRUE))
  
  # wrong x and y lengths
  expect_error(autoimage.xyz.setup(x = 1:5, y = 1:nc, z = tasmax[, , 1], 
                                   "", "", arglist, TRUE))
  expect_error(autoimage.xyz.setup(x = 1:nr, y = 1:5, z = tasmax[, , 1], 
                                   "", "", arglist, TRUE))
  ...
  expect_error(autoimage.xyz.setup(x = matrix(1, nrow = (nr + 1), ncol = nc), 
                                   y = matrix(rnorm(prod(dim(z))), nrow = nr), 
                                   z = z, "", "", arglist, TRUE, c(TRUE, TRUE)), 
               "common.legend should be a single value")
  expect_error(autoimage.xyz.setup(x = matrix(1, nrow = (nr + 1), ncol = nc), 
                                   y = matrix(rnorm(prod(dim(z))), nrow = nr),
                                   z = z, "", "", arglist, TRUE, "tom"),
