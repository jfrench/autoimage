...
#'
#' \code{paxes} addes x and y axes to an existing plot for 
#' projected coordinates.
#' 
#' The \code{\link[mapproj]{mapproject}} function is used for projection.
#' @inheritParams pimage
#' @param xlim A vector with the minimum and maximum value of 
#' the x coordinates.  Taken from \code{par("usr")} if not provided.
#' @param ylim A vector with the minimum and maximum value of 
#' the y coordinates.  Taken from \code{par("usr")} if not provided.
#' @param xaxp A vector of the form \code{c(x1, x2, n)} giving the 
#' coordinates of the extreme tick marks and the number of 
#' intervals between tick marks.  Overrides \code{xlim}.
#' @param yaxp A vector of the form \code{c(x1, x2, n)} giving the 
#' coordinates of the extreme tick marks and the number of 
#' intervals between tick marks.  Overrides \code{ylim}.
#' @param grid A logical value indicating whether grid lines
#' should be displayed with the axes.  Default is \code{TRUE}.
#' @param ... Other arguments passed to the \code{[graphics]{lines}}
#' function used to plot the grid lines.
#' @seealso \code{\link[graphics]{image}}, \code{\link[mapproj]{mapproject}}
#' @return NULL
#' @examples
#' data(narccap)
...
paxes <- function(proj, xlim, ylim, xaxp, yaxp, grid = TRUE, ...){
  if(missing(xlim)) xlim <- par("usr")[1:2]
  if(missing(ylim)) ylim <- par("usr")[3:4]
  if(missing(xaxp)) xaxp <- NULL
  if(missing(yaxp)) yaxp <- NULL
  ...
  if(proj == "none"){
    ...
    axis(1, at = xe, labels = xat)
    axis(2, at = ye, labels = yat)
  }
