#' @name copoly
#' @title Colorado state border
#' @description A list with components \code{x} and \code{y} specifying
#' the borders of the state of Colorado in longitude/latitude coordinates.  
#' This was derived from the \code{\link[maps]{stateMapEnv}} in the 
#' \code{maps} package.
#' @docType data
#' @usage data(copoly)
#' @format Contains:
#' \describe{
#'  \item{x}{longitude coordinates for Colorado border}
#'  \item{y}{latitude coordinates for Colorado border}
#' }
#' @source The \code{\link[maps]{stateMapEnv}} in the \code{maps} package.
NULL