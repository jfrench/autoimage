...
#' autoimage3(lon, lat, tasmax2, col = fields::tim.colors(12), 
#' size = c(1, 2), common.legend = FALSE)
#' 
...
#' @export
autoimage3 = function(x, y, z, legend = "horizontal", proj = "none", proj.args, 
  ...
  autolayout(size, legend = legend, common.legend = common.legend,
             ...
             reverse = FALSE)
  if(legend == "none"){ # sequence of plots with no legend
    for(i in seq_along(xyz.list)){
      par(mar = curmar)
      arglisti <- xyz.list[[i]]
      arglisti$x <- NULL; arglisti$y <- NULL; arglisti$z <- NULL
      xyz <- pimage.xyz.setup(x = xyz.list[[i]]$x, 
                              y = xyz.list[[i]]$y,
                              z = xyz.list[[i]]$z, 
                              tx = tx, ty = ty, arglist = arglisti)
      objecti <- pimage.setup(xyz, legend, proj, proj.args, lratio)
      do.call(objecti$plotf, objecti$arglist)
    }
    deficit <- prod(size) - ng
    for(i in seq_len(deficit)){ blank.plot() }
  }else{
    if(common.legend){ # sequence of plots with common legend
      # plot sequence of plots
      for(i in seq_along(xyz.list)){
        par(mar = curmar)
        arglisti <- c(xyz.list[[i]], legend = "none", 
                      proj = proj, proj.args = proj.args,
                      map = "none")
        do.call("pimage", arglisti)
      }
      deficit <- prod(size) - ng
      for(i in seq_len(deficit)){ blank.plot() }
      if(common.legend) autolegend() 
    }else{ # sequence of plots without common.legend
      for(i in seq_along(xyz.list)){
        par(mar = curmar)
        arglisti <- xyz.list[[i]]
      ...
      deficit <- prod(size) - ng
      for(i in seq_len(2*deficit)){ blank.plot() }
    }
  }
