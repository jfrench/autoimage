...
autoimage.xyz.setup <- function(x, y, z, tx, ty, arglist, verbose, common.legend = FALSE){
  ...
  if(!is.matrix(z) & !is.array(z)){
    ...
    # dimension or has correct spacing for regular grid or
    # has irregularly spaced coordinates of proper dimension
    ...
    if(!is.matrix(y)){
      if(length(y) != ncol(z) & length(y) != nrz){
  ...
  if(is.matrix(z)){
    if(is.matrix(x)){
      ...
      arglist$x <- x; arglist$y <- y; arglist$z = z
      xyz.list[[1]] <- arglist
      # xyz.list[[1]] <- list(x = x, y = y, z = z, arglist = arglist)
      if(verbose){
      ...
      for(i in seq_len(ncol(z))){
        ...
        arglist0$x <- x; arglist0$y <- y; arglist0$z <- z[,i]
        xyz.list[[i]] <- arglist0
      }
      if(verbose){
  ...
  } else if (is.array(z)){ # sequence of images
    n3 <- dim(z)[3]
    ...
    if(length(x) == nrow(z)){ # regular grid
      if(verbose){
  ...
  } else{ # single irregularly spaced
    xyz.list <- vector("list", 1)
    arglist$x <- x; arglist$y <- y; arglist$z = z
    xyz.list[[1]] <- arglist
    # xyz.list[[i]] <- list(x = x, y = y, z = z, arglist = arglist)
    if(verbose){
